...
package write_tests //nolint:revive // integration test package
...
func TestRegisterAndExecuteViaRegistry(t *testing.T) {
	...
	if !found {
		t.Fatal("write tool not found in registry List()")
	}

	// Verify Get() returns the tool definition with correct fields.
	def, ok := reg.Get("write")
	if !ok {
		t.Fatal("write tool not found via Get()")
	}
...
func TestRegisterWithAllowedDirs(t *testing.T) {
	...
	if err := write.Register(reg, write.WithAllowedDirs(allowedDir)); err != nil {
...
func TestWriteMalformedJSON(t *testing.T) {
	...
	result, err := reg.Execute(context.Background(), "write", `{invalid json`)
	if err != nil {
		t.Fatalf("Execute returned error: %v", err)
	}
...
func TestWriteExceedsMaxWriteBytes(t *testing.T) {
	...
	reg := tool.NewRegistry()
	if err := write.Register(reg, write.WithMaxWriteBytes(10)); err != nil {
		t.Fatalf("Register failed: %v", err)
	}
...
func TestWriteContextCancellation(t *testing.T) {
	...
	result, err := reg.Execute(ctx, "write",
...
func TestWriteErrorThenSuccess(t *testing.T) {
	...
	result1, err := reg.Execute(context.Background(), "write",
		`{"file_path":"","content":"data"}`)
	if err != nil {
		t.Fatalf("Execute returned error: %v", err)
	}
	...
	result2, err := reg.Execute(context.Background(), "write",
