...
func TestRegisterAndExecuteViaRegistry(t *testing.T) {
	...
	if !found {
		t.Fatal("edit tool not found in registry List()")
	}

	// Verify Get() returns the tool definition with correct fields.
	def, ok := reg.Get("edit")
	if !ok {
		t.Fatal("edit tool not found via Get()")
	}
	...
	result, err := reg.Execute(context.Background(), "edit",
...
func TestRegisterWithAllowedDirs(t *testing.T) {
	...
	if err := edit.Register(reg, edit.WithAllowedDirs(allowedDir)); err != nil {
...
func TestEditMalformedJSON(t *testing.T) {
	...
	result, err := reg.Execute(context.Background(), "edit", `{invalid json`)
	if err != nil {
		t.Fatalf("Execute returned error: %v", err)
	}
...
func TestEditExceedsMaxFileBytes(t *testing.T) {
	...
	reg := tool.NewRegistry()
	if err := edit.Register(reg, edit.WithMaxFileBytes(100)); err != nil {
		t.Fatalf("Register failed: %v", err)
	}
...
func TestEditContextCancellation(t *testing.T) {
	...
	result, err := reg.Execute(ctx, "edit",
...
func TestToolDefSchemaForLLMCompatibility(t *testing.T) {
	...
	if aiTools[0].Function.Name != "edit" {
		t.Errorf("expected function name 'edit', got %q", aiTools[0].Function.Name)
	}
}
...
func TestEditErrorThenSuccess(t *testing.T) {
	...
	result1, err := reg.Execute(context.Background(), "edit",
	...
	result2, err := reg.Execute(context.Background(), "edit",
...
func TestCombinedReadWriteEditWorkflow(t *testing.T) {
	...
	editResult, err := reg.Execute(context.Background(), "edit",
		fmt.Sprintf(`{"file_path":%q,"old_string":"hello","new_string":"goodbye"}`, path))
	if err != nil {
