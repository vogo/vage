...
package bash_tests //nolint:revive // integration test package
...
func TestRegisterAndExecuteViaRegistry(t *testing.T) {
	...
	if err := bash.Register(reg); err != nil {
...
func TestRegisterWithCustomOptions(t *testing.T) {
	...
	if err := bash.Register(reg, bash.WithWorkingDir("/tmp")); err != nil {
...
func TestExecuteTimeout(t *testing.T) {
	reg := tool.NewRegistry()
	if err := bash.Register(reg, bash.WithTimeout(200*time.Millisecond)); err != nil {
		t.Fatalf("Register failed: %v", err)
	}
...
func TestExecuteContextCancellation(t *testing.T) {
	reg := tool.NewRegistry()
	if err := bash.Register(reg, bash.WithTimeout(30*time.Second)); err != nil {
		t.Fatalf("Register failed: %v", err)
	}
...
func TestExecuteOutputTruncation(t *testing.T) {
	...
	if err := bash.Register(reg, bash.WithMaxOutput(maxBytes)); err != nil {
		t.Fatalf("Register failed: %v", err)
	}
...
func TestExecuteProcessGroupCleanup(t *testing.T) {
	reg := tool.NewRegistry()
	if err := bash.Register(reg, bash.WithTimeout(500*time.Millisecond)); err != nil {
		t.Fatalf("Register failed: %v", err)
	}
...
func TestExecuteWithWorkingDirectory(t *testing.T) {
	...
	tmpDir, err := os.MkdirTemp("", "bash-integration-*")
	if err != nil {
	...
	reg := tool.NewRegistry()
	if err := bash.Register(reg, bash.WithWorkingDir(tmpDir)); err != nil {
		t.Fatalf("Register failed: %v", err)
	}
