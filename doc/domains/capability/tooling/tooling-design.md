...
- **Schema inference has one construction path and two failure modes**: `Compile` is the only entry that returns a construction error, so dynamic catalogs can reject one illegal type without `recover`. `MustInfer` panics the same formatted string `Infer` always did; `Infer` delegates to it for source compatibility. All three share the same reflection/schema/handler assembly. Failure returns a zero definition and a nil handler so ignoring the error cannot register a partial tool. The inferable Go type set does not grow. Do not wrap unknown panics into construction errors.
