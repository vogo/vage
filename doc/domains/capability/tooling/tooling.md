...
| TOOL-11 | **Schema-inference construction is recoverable at the catalog boundary**: dynamic or batch registration uses `Compile`, which returns an error for an illegal parameter type so the caller can skip that definition and continue. Static declarations use `MustInfer`, which panics because a bad type is a programming mistake. `Infer` remains the compatibility panic entry with unchanged signature and panic payload. Construction failure must not leave a registerable definition or handler. Schema inference range does not expand (see [tooling-design](tooling-design.md)). |
