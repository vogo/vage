...
- **恢复保存的是已解析参数与已消耗预算而非重新取默认值**:`interrupt.EffectiveParams` 快照模型、温度、最大迭代、预算、工具过滤、停止序列,记录另存挂起前 budget tracker 的已消耗 token——`ResumeInterrupt` 用它们重建 `runParams` 与剩余预算,不调用 `resolveRunParams(nil)` 重新合并新进程的 Agent 默认值。这样跨中断仍属于同一逻辑 Run,新进程的配置变化或 tracker 重建不会额外赠送模型轮次。
...
- **空决定按记录状态解释**:`ResumeInterrupt` 省略 `Decisions` 时不提交新决定。记录仍为 Pending 时只返回剩余待决项且不启动工具或模型;记录已由此前调用推进到 Ready 时则尝试获取租约并继续恢复,使工具或模型失败后的重试无需再次提交人工决定。
