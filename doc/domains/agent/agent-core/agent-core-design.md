...
| `agent/taskagent` | `New` + 一整套 `With*` 选项、`Quick`、`Run`/`RunStream`/`Resume`、`WithMiddleware`、`WithInterruptStore`/`WithInterruptPolicy`/`WithInterruptToolNames`/`ResumeInterrupt` | ReAct loop and integration hub; `New + Option` is the full constructor contract, `Quick` is a thin wrapper over its common arguments. Interrupt suspend/resume: design decisions below. |
...
- **Resume restores resolved parameters and consumed budget, not Agent defaults**: `interrupt.EffectiveParams` snapshots model, temperature, max iterations, budget, tool filter and stop sequences; the record also stores the budget tracker’s consumed tokens. `ResumeInterrupt` rebuilds `runParams` and remaining budget from those, and does not call `resolveRunParams(nil)` against the new process’s Agent defaults. The two halves remain one logical Run; a config change or a fresh tracker cannot gift extra model turns.
...
- **Lease, not a permanent lock, so a crashed resumer can be taken over**: `interrupt.Store.AcquireLease` is owner + expiry. `FileStore` uses an `<id>.lock` file (`O_CREATE|O_EXCL`) held only for one critical section; every mutation including `Delete` takes that lock. Lease identity and expiry live on the Record; an expired lease is reclaimable. This is not end-to-end exactly-once — after takeover, ordinary tools that have not reached the next durable boundary may replay; see [orchestration](../orchestration/orchestration.md).
...
- **可恢复性**:Resume 复用与 Run 相同的循环与 finalize 路径,保证续跑与首跑行为一致. With neither `WithInterruptStore` nor `WithInterruptPolicy` configured, the interrupt gate is a no-op (zero-cost path); configuring only one of them is a construction error, not a silent downgrade — interrupt has no "partially enabled" safe state.
