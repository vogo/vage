...
| AC-16 | **Interrupt 只在顶层可恢复**:一次挂起只有直接调用 TaskAgent `Run`、拿到 `RunResponse.Interrupt` 的宿主能组织人工决策并 `ResumeInterrupt`。框架内置的父层消费路径 —— agent-as-tool、RouterAgent、WorkflowAgent 顺序步骤、orchestrate 的 DAG 节点 / 循环体 / 条件节点 / DynamicSpawn 父子 Runner / 前向恢复、`workflow.AdaptRunner` —— 都不提供 resume 通路,因此在读取或改写子响应的任何输出**之前**判定 `StopReason == interrupted` **或** `Interrupt != nil`(`RunResponse.IsInterrupted()`),任一命中即拒绝:agent-as-tool 返回 `IsError=true` 的工具结果,其余返回 Go error。拒绝的错误说明「nested HITL 不受支持」,不复制半截 assistant 文本、不转发 `InterruptDescriptor`;父层不询问人类、不调用 `ResumeInterrupt`、不改选路由或跳过步骤。两个信号任一成立即足以证明该响应不可消费,自定义 Agent 或 middleware 产生不一致组合时不得靠缺少另一字段绕过。拒绝发生在子 Agent 已创建 interrupt **之后**,因此它不阻止 Pending 记录产生、也不清理该记录;要恢复就必须由顶层直接运行该 Agent。`max_iterations_exceeded`、`token_budget_exhausted` 等其他停止原因不在此列,照旧按正常响应消费。 |
...
| AC-18 | **流消费与事件折叠的诚实边界**:`RunStream.ForEach` 排空至流结束,`io.EOF` 被消费并报告为成功;其余 `Recv` 错误原样上抛(保持 `errors.Is`/`As` 可判定);回调返回错误立即停止投递并原样返回,此后事件不再送达。无论哪条路径,返回前流都已关闭。`EventAccumulator` 折叠交给它的每一个事件,不按 `AgentID` / `ParentID` 过滤 —— 混流的取舍由调用方在 Add 之前完成。它对「不知道」与「是零」严格区分:流中没有 `LLMCallEnd` 时 `Usage` 为 nil 而非零值 Usage,没有 `AgentEnd` 时 stop reason 为空、耗时为 0。**运行的答案以 `AgentEnd.Message` 为准**(AC-11 的同一条规则):只要 `AgentEnd` 到达就以它为答案,包括它报告为空的情形 —— 护栏与 middleware 的改写只体现在这里,已发出的 delta 不被重放,而经 `RunToStream` 包装的非流式 Agent 根本不产生 delta;只有从未到达 `AgentEnd` 的流(报错、取消、提前弃读)才退回累积的 delta 作为尽力而为的近似。`Text()` 仍报告流上实际出现过的 delta,与答案是两个问题。`Response` 是展示级重建,不承诺等同于同一次运行的 `Run` 返回值:至多一条 assistant 文本消息、`Interrupt` 恒为 nil,provider native 载荷与逐消息 tool_call 结构不参与重建。未识别的事件类型被忽略而非报错,以便新增事件类型不破坏既有消费者。 |
| AC-19 | **Run 参数解析是单槽参数化,不是新平面**:`taskagent.WithParamResolver` 在输入护栏之后、工具冻结与 `prepareContext` 之前,对每个新的 `Run`/`RunStream` 恰好调用一次;后写覆盖先写,框架不排序、不链式组合。Resolver 可收紧 model、limits、`ToolMode` 与候选工具,并写入不透明审计 `subject`;框架不定义 `auth.Principal`,也不在此处换 Caller/endpoint。`Resume`/`ResumeInterrupt` 不调用 Resolver:interrupt 从 v2 快照恢复冻结工具名,checkpoint Resume 仍用 Agent 默认参数。工具可见性按「请求范围 ∩ skill `AllowedTools` ∩ `EnabledFunc`」只能收紧;冻结空集不得回退全集。多凭证/多 endpoint 由宿主在 Agent 构造期绑定 `largemodel.BuildCaller` 的 `ComposeCaller`(见 [0003](../../../architecture/adr/0003-run-param-resolver.md))。 |
...
TaskAgent declares an internal **entry policy** for each run-class entry (`Run`, `RunStream`, `Resume`, `ResumeInterrupt`). The policy controls four cross-cut layers; caller validation and interrupt-config checks (`checkInterruptConfig`, AC-17) always run on every entry via shared preflight regardless of policy flags.

| Entry | Run values (`WithRunValues`) | Input guards | Agent middleware (`WithMiddleware`) | ParamResolver | Caller + interrupt preflight |
|-------|:----------------------------:|:------------:|:-----------------------------------:|:-------------:|:----------------------------:|
| `Run` / `RunStream` | yes | yes | yes | yes (once) | yes |
| `Resume` | yes | no | no | no | yes |
| `ResumeInterrupt` | yes | no | no | no | yes |

Fresh-run entries enable the policy-controlled cross-cuts, including one ParamResolver call after input guards and before context build. Resume-class entries enable run-values initialization only; middleware, input guards and ParamResolver are explicitly off because the original run already passed them and interrupt parameters come from the v2 snapshot (ADR 0001, ADR 0003, AC-10, AC-14). Output guards, tool-result guards, hook dispatch, `AgentStart` timing, and stream event ordering are **not** entry-policy flags — they follow existing per-path rules unchanged.
...
- **模型看见工具定义前收窄本次 Run 的模型、限额或工具披露**(AC-19) → `taskagent.WithParamResolver`。这是 preflight 的单槽参数化,不是第五条装饰链;不要把它放进 Agent Middleware,也不要把 Caller/endpoint 放进 `RunRequest`。
...
任务型 Agent 在整条 ReAct 路径上发出结构化事件:AgentStart/End、IterationStart、TextDelta(流式)、ToolCall/ToolResult、LLM 调用、Token 预算、护栏、技能、编排、上下文构建、参数解析(`params_resolved`; model、limits、tool_mode、工具数量与名称 SHA-256、脱敏 subject、`resolver_touched` —— 不含工具名列表或凭证)、路由选路(`route_selected`; endpoint alias、strategy、reason、stream —— 不含配置或密钥)、interrupt (`interrupt_created` / `interrupt_decision_stored` / `interrupt_resumed`; identity, status, tool-call IDs and timing only — no decision or message body). 消费者为 `platform` 组的 hook 与流式调用方。
