...
| AC-15 | **多模态输入仅对用户消息有效**:`MessagePartImage`/`MessagePartFile` 只能出现在 `RoleUser` 消息,且每个 part 恰好一个来源(URL、Data,文件另可 FileID);缺失/多个来源、内联 Data 缺 MimeType、图片 MimeType 非 `image/*`、错误角色、跨类型字段均在 `Message.Validate()` 阶段失败,不静默丢弃媒体或改写来源。反向亦然:媒体字段(URL / Data / MimeType / FileID / Filename)只能出现在 image/file part 上 —— 文本、thinking、tool_call、tool_result part 携带任一媒体字段同样在 `Validate()` 失败,因为 codec 只从 image/file part 读取来源,放行等于静默丢弃。某个来源能否进入具体 provider wire 由对应 codec 校验(见 [model.md](../../capability/model/model.md)):不可表示的组合(OpenAI 文件 URL、Anthropic FileID)在 backend 调用前明确报错;Anthropic document block 丢弃 Filename 是唯一允许的已知降级。 |
