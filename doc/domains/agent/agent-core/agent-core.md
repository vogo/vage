...
| AC-19 | **Run 参数解析是单槽参数化,不是新平面**:`taskagent.WithParamResolver` 在输入护栏之后、工具冻结与 `prepareContext` 之前,对每个新的 `Run`/`RunStream` 恰好调用一次;后写覆盖先写,框架不排序、不链式组合。Resolver 可收紧 model、limits、`ToolMode` 与候选工具,并写入不透明审计 `subject`;框架不定义 `auth.Principal`,也不在此处换 Caller/endpoint。`Resume`/`ResumeInterrupt` 不调用 Resolver:interrupt 从 v2 快照恢复冻结工具名,checkpoint Resume 仍用 Agent 默认参数。工具可见性按「请求范围 ∩ skill `AllowedTools` ∩ `EnabledFunc`」只能收紧;冻结空集不得回退全集。多凭证/多 endpoint 由宿主在 Agent 构造期绑定 `largemodel.BuildCaller` 的 `ComposeCaller`(见 [0002](../../../architecture/adr/0002-run-param-resolver.md))。 |
