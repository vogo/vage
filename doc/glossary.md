...
| **ParamResolver** | TaskAgent 构造期单槽:在输入护栏之后、上下文与工具冻结之前,收紧本次新 Run 的模型、限额与工具范围。后写覆盖先写,不可链;`Resume` 不调用。见 [agent-core](domains/agent/agent-core/agent-core.md) AC-19 与 [ADR 0002](architecture/adr/0002-run-param-resolver.md)。 |
