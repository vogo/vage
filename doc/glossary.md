...
| **StopReason(终止原因)** | 标记一次 Agent 调用为何结束:wire 值为 `complete`(完成)/ `max_iterations_exceeded`(达上限)/ `token_budget_exhausted`(预算耗尽)/ `interrupted`(本次调用结束、逻辑 Run 尚未终结,见 Interrupt)。 |
...
| **中断(Interrupt)** | `interrupt` 包提供的第三种持久执行状态:策略在工具批执行前命中一个或多个工具调用时,框架冻结整批、持久化 `interrupt.Record` 并以 `StopReasonInterrupted` 结束本次调用(逻辑 Run 尚未终结)。`TaskAgent.ResumeInterrupt(ctx, req)` 按 `interrupt_id + tool_call_id` 精确注入外部决定,从该工具批继续。与相邻概念的边界:不同于 `ask_user`(处理器已开始执行、进程内同步等待、无框架级挂起记录),也不同于 checkpoint(迭代已完成或 Run 已终止后的崩溃重放快照、按 `Resume(sessionID)` 取最近完整轮次续跑)。三者互不替代,详见 [agent-core](domains/agent/agent-core/agent-core.md) 与 [orchestration](domains/agent/orchestration/orchestration.md)。 |
