...
- **大模型接入**:唯一入口是 `largemodel` 的 `Caller` 协议调用层,其下直连 `github.com/vogo/aimodel` 各 provider 的 native 客户端(OpenAI Chat / OpenAI Responses / Anthropic Messages)。厂商协议细节只允许出现在 `largemodel` 的 provider 实现与 `schema` 的 wire 承载类型中,不得散落到其他核心包。
