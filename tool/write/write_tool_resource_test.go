...
func TestWriteTool_ResourceIDs_HappyPath(t *testing.T) {
	wt := New()
	got := wt.ResourceIDs(map[string]any{"file_path": "/abs/path/to/file.go"})
...
func TestWriteTool_ResourceIDs_CleansPath(t *testing.T) {
	wt := New()
	got := wt.ResourceIDs(map[string]any{"file_path": "/abs/./a//b/../c.go"})
	if len(got) != 1 {
		t.Fatalf("len = %d, want 1", len(got))
	}
...
func TestWriteTool_ResourceIDs_Defensive(t *testing.T) {
	wt := New()
	cases := []struct {
		name string
		args map[string]any
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := wt.ResourceIDs(tc.args); got != nil {
